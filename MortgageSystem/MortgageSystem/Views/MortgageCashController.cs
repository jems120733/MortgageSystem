...
namespace MortgageSystem.Views
{
    public class MortgageCashController : Controller
    {
        ...
        public async Task<ActionResult> Edit(long? id)
        {
            ...
            //ViewBag.crm_from_branch_id = new SelectList(db.crm_branch, "id", "description", trans_transaction_header.crm_from_branch_id);
            //ViewBag.crm_to_branch_id = new SelectList(db.crm_branch, "id", "description", trans_transaction_header.crm_to_branch_id);
            //ViewBag.crm_customer_id = new SelectList(db.crm_customer, "id", "first_name", trans_transaction_header.crm_customer_id);
            //ViewBag.crm_user_id = new SelectList(db.crm_user, "id", "username", trans_transaction_header.crm_user_id);
            //ViewBag.crm_admin_id = new SelectList(db.crm_user, "id", "username", trans_transaction_header.crm_admin_id);
            //ViewBag.inv_discount_id = new SelectList(db.inv_discount, "id", "description", trans_transaction_header.inv_discount_id);
            //ViewBag.mf_document_type_id = new SelectList(db.mf_document_type, "id", "description", trans_transaction_header.mf_document_type_id);
            ...
            //ViewBag.trans_transaction_type_id = new SelectList(db.trans_transaction_type, "id", "description", trans_transaction_header.trans_transaction_type_id);
        ...
        // POST: MortgageCash/Edit/5
        // To protect from overposting attacks, please enable the specific properties you want to bind to, for 
        // more details see http://go.microsoft.com/fwlink/?LinkId=317598.
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Edit([Bind(Include = "id,trans_transaction_type_id,sales_date,date_created,mf_document_type_id,crm_branch_id,crm_from_branch_id,crm_to_branch_id,crm_customer_id,crm_user_id,crm_admin_id,inv_discount_id,mf_is_void_status_id,mf_open_status_id,discount_amount,comment")] trans_transaction_header trans_transaction_header)
        {
            if (ModelState.IsValid)
            {
                db.Entry(trans_transaction_header).State = EntityState.Modified;
                await db.SaveChangesAsync();
                return RedirectToAction("Index");
            }
            ViewBag.crm_branch_id = new SelectList(db.crm_branch, "id", "description", trans_transaction_header.crm_branch_id);
